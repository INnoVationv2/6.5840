...
func (sc *ShardCtrler) getMatchIndex(clientId int64) int32 {
	sc.mu.RLock()
	defer sc.mu.RUnlock()

	return sc.matchIndex[clientId]
}

func (sc *ShardCtrler) setMatchIndex(clientId int64, newMatchIndex int32) {
	sc.matchIndex[clientId] = maxi32(sc.matchIndex[clientId], newMatchIndex)
	DPrintf("[%v]Update Match Index To %d", sc.getServerDetail(), sc.matchIndex[clientId])
}
